...
class SqlCommand {
	...
	public function DateDBBRT($date) {
		if($date==null) 
				return NULL;
		if(strlen($date)<10 )
			return(NULL);
		...
		return(sprintf('%02d/%02d/%4d', $d, $m, $y)." $tt");
	}
	...
	public function Execute() {
		...
		if($this->params!=NULL) {
			...
			if($this->error == NULL) $Result = pg_query_params($this->connection,$this->query, $this->params);
                        else $Result = @pg_query_params($this->connection,$this->query, $this->params);
		}
		else {
			if($this->error == NULL) $Result = pg_query($this->connection,$this->query);
                        else $Result = @pg_query($this->connection,$this->query);
		}
		if($Result==false) {	
			$this->exception = 	pg_last_error();
                }
