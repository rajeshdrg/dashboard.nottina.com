    ...
    fetch('/ShowAlerta/selectAn.php?action=obter_analistas')
        ...
        .then(data => {
            if (data.success) {
                ...
                data.analistas.forEach(analista => {
                    const option = document.createElement('option');
                    option.value = sanitizeInput(analista.cod_usuario);
                    option.textContent = sanitizeInput(analista.nome);
                    select.appendChild(option);
                });
            } else {
                console.error('Erro ao obter analistas:', data.message);
...
form.addEventListener("submit", event => {
    ...
    var formData = new FormData(form);
    var formObject = {
        cod_alerta: sanitizeInput(formData.get('cod_alerta')),
        analista: sanitizeInput(formData.get('analista')),
        fechamento: sanitizeInput(formData.get('fechamento'))
    };
