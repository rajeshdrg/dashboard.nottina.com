...
class GuardarEdicion {
    ...
    function guardarEdicion($codAlerta, $quando, $analista) {
        ...
        try {
            $Sql->Execute();
            echo json_encode(array('success' => true, 'message' => 'Os campos foram alterados corretamente.'));
        } catch (Exception $e) {
            echo json_encode(array('success' => false, 'message' => 'Erro ao executar consulta: ' . $e->getMessage()));
        }
        exit();
    }
}
